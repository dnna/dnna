 ...
 */
class Aueb_Action_Helper_CreateIcal extends Zend_Controller_Action_Helper_Abstract {
    ...
    protected function addEvent(Aueb_Model_Entry $event, &$v, $controller, $unique_id) {
        $vevent = & $v->newComponent('vevent');
        // create an event calendar component
        $vevent->setProperty("uid", spl_object_hash($event)."@".$unique_id);
        ...
        $vevent->setProperty('LOCATION', $event->get_room());
        // property name - case independent
        $vevent->setProperty('summary', $event->__toString());
        $vevent->setProperty('description', $event->get_prof());
        $vevent->setProperty('comment', $event->__toString());
        $vevent->setProperty('attendee', $this->_email);
        $vevent->setProperty("rrule", array("FREQ" => "WEEKLY", "COUNT" => 14, "INTERVAL" => 1)); // Recur every week
